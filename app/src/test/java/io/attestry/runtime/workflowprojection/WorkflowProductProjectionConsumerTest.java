...
class WorkflowProductProjectionConsumerTest {
    ...
    private WorkflowProjectionDlqPublisher dlqPublisher;
    ...
    void setUp() {
        ...
        WorkflowProjectionMetrics metrics = new WorkflowProjectionMetrics(new SimpleMeterRegistry());
        ProjectionPayloadMapper payloadMapper = new ProjectionPayloadMapper();
        ...
        dlqPublisher = new WorkflowProjectionDlqPublisher(kafkaTemplate, properties, metrics);
        consumer = new WorkflowProductProjectionConsumer(
            new ProjectionEventParser(new ObjectMapper()),
            dlqPublisher,
            metrics,
            List.of(
                new ProductProjectionRefreshHandler(projectionWriter, metrics, payloadMapper),
                new ShipmentProjectionRefreshHandler(shipmentProjectionWriter, metrics, payloadMapper),
                new DistributionProjectionRefreshHandler(distributionProjectionWriter, metrics, payloadMapper),
                new RetailAccessProjectionRefreshHandler(retailAccessProjectionWriter, metrics, payloadMapper)
            )
        );
    }
