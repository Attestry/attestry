...
-- >>> V10: ledger entry append only trigger
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_ledger_entry_no_update') THEN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_ledger_entry_no_delete') THEN
...
CREATE TABLE IF NOT EXISTS workflow_service_requests (
    ...
    CONSTRAINT chk_svc_status CHECK (status IN ('SUBMITTED','COMPLETED','CANCELLED'))
);

CREATE UNIQUE INDEX IF NOT EXISTS uq_svc_passport_submitted ON workflow_service_requests (passport_id) WHERE status = 'SUBMITTED';
...
-- >>> V15: ledger payload json to jsonb
