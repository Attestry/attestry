package io.attestry.userauth.infrastructure.persistence.jdbc.outbox;
...
import io.attestry.userauth.infrastructure.persistence.jpa.notification.JpaNotificationOutboxWriteRepository;
...
public class JdbcNotificationOutboxOperations implements NotificationOutboxOperationsPort {

    private final NotificationOutboxJpaRepository repository;
    private final JpaNotificationOutboxWriteRepository writeRepository;
    ...
    public List<NotificationOutbox> claimPendingRetryable(Instant now, int batchSize, String processingOwner) {
        ...
        return repository.findRetryable(ids).stream().map(writeRepository::toDomain).toList();
    }
