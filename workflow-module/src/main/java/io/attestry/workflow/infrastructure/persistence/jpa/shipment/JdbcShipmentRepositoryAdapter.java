...
public class JdbcShipmentRepositoryAdapter implements ShipmentRepository {
    ...
    private static final String RELEASED_STATUS = ShipmentStatus.RELEASED.name();
    ...
    public boolean existsActiveReleasedByPassportId(String passportId) {
        Integer count = jdbcTemplate.queryForObject(
                "SELECT COUNT(1) FROM workflow_shipments WHERE passport_id = ? AND status = ?",
                Integer.class,
                passportId,
                RELEASED_STATUS);
        ...
        jdbcTemplate.update(
                """
                            INSERT INTO workflow_shipments (
                                ...
                                released_at, released_by_user_id, released_by_tenant_id,
